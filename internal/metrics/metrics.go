...
func New() *Metrics {
	...
	m := &Metrics{
		...
		SpoolSaved:             counter("spool_records_saved_total", "Records durably written to the batch WAL before their first ClickHouse insert attempt."),
		...
		SpoolLost:              counter("spool_records_lost_total", "Records lost after both ClickHouse and the write-ahead log were unavailable. THIS IS PERMANENT EVIDENCE LOSS."),
		SpoolFiles:             gauge("spool_files", "Active and replayable batches currently protected by the disk WAL."),
		SpoolBytes:             gauge("spool_bytes", "Bytes held by active, replayable, and quarantined WAL evidence."),
