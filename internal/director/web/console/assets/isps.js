...
(function($){
 ...
 function field(key,type,value,required,hint=''){
  const id='isp-'+key;
  return `<div class="isp-field"><label class="lbl" for="${id}">${labels[key]}${required?' <span class="req" aria-hidden="true">*</span>':''}</label><input class="inp" id="${id}" name="${key}" type="${type}" value="${esc(value||'')}" ${required?'required':''} ${key==='Username'?'autocomplete="off" maxlength="64"':key==='Email'?'autocomplete="email" maxlength="190"':key==='Phone'?'autocomplete="tel" maxlength="25"':type==='password'?'autocomplete="new-password" maxlength="72"':'maxlength="190"'} aria-describedby="${id}-hint ${id}-error"><div class="hint" id="${id}-hint">${hint}</div><div class="isp-field-error" id="${id}-error"></div></div>`;
 }
 ...
 function edit(ctx,i){
  ...
  const d=dialog(ctx,i?'Edit ISP':'Create ISP',i?`${i.Name} · ISP #${i.ID}`:'Tenant details and primary login',`<p class="isp-form-note">All marked fields are required. The primary user can sign in with their username or email.${i&&!needsPassword?' Leave both password fields blank to keep the current password.':''}</p><div class="isp-fields">${field('Name','text',i?.Name,true)}${field('Username','text',i?.Username,true,'3–64 characters; unique across ISPs.')}${field('Email','email',i?.Email,true,'Primary login and contact email.')}${field('Phone','tel',i?.Phone,true,'Include the country code when available.')}${field('Password','password','',needsPassword)}${field('ConfirmPassword','password','',needsPassword)}<div class="isp-field"><label class="lbl" for="isp-Enabled">Status <span class="req" aria-hidden="true">*</span></label><select class="inp" id="isp-Enabled" name="Enabled" required aria-describedby="isp-Enabled-hint isp-Enabled-error"><option value="">Choose status</option><option value="true" ${i?.Enabled===true?'selected':''}>Active</option><option value="false" ${i?.Enabled===false?'selected':''}>Disabled</option></select><div class="hint" id="isp-Enabled-hint">Blocks ISP user access. Manage exporter collection on the Devices page.</div><div class="isp-field-error" id="isp-Enabled-error"></div></div></div>`,`<button type="submit" class="btn">${icon('save')}${i?'Save changes':'Create ISP'}</button>`);
  let tried=false;
